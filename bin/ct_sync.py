...
def build_index(scripts_dir: Path):
    """Map base name -> repo script. Returns (index, conflicts).
    ...
    for path in sorted(scripts_dir.rglob("*.py")):
        if UNMATCHED in path.relative_to(scripts_dir).parts:
            continue
        key = base_name(path.stem)
        if key in index:
            conflicts.setdefault(key, [index[key]]).append(path)
        else:
            index[key] = path
...
def has_magic(text: str, magic: str) -> bool:
    """True if the file's first real line is the marker.
    ...
    which one you type should not matter.
    """
    if not magic:
        return False
    _, line = _first_real_line(text)
    return line is not None and _unquote_marker(line).lower() == magic.lower()
...
def tidy_unmatched(index, opts: Options) -> None:
    ...
    for path in sorted(opts.unmatched_dir.glob("*.py")):
        if base_name(path.stem) not in index:
            continue
        text = read(path)
        ...
        if opts.dry_run:
            typer.echo("  would drop %-24s (blank, now matched by %s)"
                       % (show(path), show(index[base_name(path.stem)])))
            continue
        try:
            path.unlink()
            typer.echo("  drop  %-28s blank, now matched by %s"
                       % (show(path), show(index[base_name(path.stem)])))
        except OSError:
            pass
...
def make_opts(save_dir, scripts_dir, strict, dry_run, verbose, no_renumber, magic) -> Options:
    save = resolve_save(save_dir)
    if not scripts_dir.is_dir():
        err("Not a directory: %s" % scripts_dir)
        raise typer.Exit(2)
    return Options(save, scripts_dir, strict, dry_run, verbose, not no_renumber, magic)
...
def status(save_dir: Optional[Path] = SaveOpt, scripts_dir: Path = ScriptsOpt,
    ...
    opts = make_opts(save_dir, scripts_dir, strict, False, False, no_renumber, magic)
...
def once(save_dir: Optional[Path] = SaveOpt, scripts_dir: Path = ScriptsOpt,
    ...
    opts = make_opts(save_dir, scripts_dir, strict, dry_run, verbose, no_renumber, magic)
    index, conflicts = build_index(opts.scripts_dir)
    report_conflicts(conflicts)
...
class Watcher:
    ...
    def rebuild(self):
        ...
        for key in sorted(set(self.index) - set(before)):
            ok("  repo  + %-22s %s" % (key, show(self.index[key])))
        for key in sorted(set(before) - set(self.index)):
            warn("  repo  - %-22s (was %s)" % (key, show(before[key])))
        for key in sorted(k for k in set(self.index) & set(before)
                          if self.index[k] != before[k]):
            typer.echo("  repo  ~ %-22s %s" % (key, show(self.index[key])))
        report_conflicts(self.conflicts)
